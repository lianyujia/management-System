...
if (isset($_POST['docsub1'])) {
    ...
    $query = "SELECT doc_id, email, email_iv, username FROM doctb";
    $result = mysqli_query($con, $query);
    ...
    if ($emailFound && $docId !== null) {
        // delete the doctor record
        $deleteQuery = "DELETE FROM doctb WHERE doc_id = ?";
        ...
        if ($stmt->execute()) {
            ...
            // log the activity
            $activity = "Removed Doctor: $doctorName";
            $encryptedActivity = encryptData($activity);
            ...
            try {
                ...
                $mail->setFrom($_ENV['EMAIL_USERNAME'], 'Admin'); // sender email and name
                $mail->addAddress($demail); // recipient email
                ...
                $mail->send();
                echo "<script>alert('Notification email sent to the doctor.');</script>";
            } catch (Exception $e) {
                echo "<script>alert('Failed to send notification email: " . $mail->ErrorInfo . "');</script>";
            }
        } else {
            echo "<script>alert('Unable to delete the doctor!');</script>";
        }
    } else {
        echo "<script>alert('Doctor with the entered email not found!');</script>";
    }
  }
