...
$query = "SELECT doc_id, username, email, email_iv, spec, spec_iv, docFees, doc_Fees_iv FROM doctb WHERE doc_id = '$doctorId'";
$result = mysqli_query($con, $query);

if ($row = mysqli_fetch_assoc($result)) {
    $decryptedEmail = decryptData($row['email'], $row['email_iv']);
    $doctorName = $row['username'];
    $spec = decryptData($row['spec'], $row['spec_iv']);
    $fee = decryptData($row['docFees'], $row['doc_Fees_iv']);
} else {
    $decryptedEmail = '';
    $doctorName = '';
    $spec = '';
...
if (isset($_POST['validate_otp'])) {
    ...
    if (empty($enteredOtp)) {
        ...
        // update password
        $updateQuery = "UPDATE doctb SET password = ? WHERE doc_id = ?";
        $stmt = $con->prepare($updateQuery);
        $stmt->bind_param("ss", $hashedPassword, $doctorId);
        ...
        <div class="card-body">
          <form method="post">
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="name" value="<?php echo htmlspecialchars($doctorName); ?>" readonly>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="spec" value="<?php echo htmlspecialchars($spec); ?>" readonly>
            </div>
            <div class="form-group">
              <label for="fee">Consultation Fees</label>
              <input type="text" class="form-control" id="fee" value="<?php echo htmlspecialchars($fee); ?>" readonly>
            </div>
            <div class="form-group">
